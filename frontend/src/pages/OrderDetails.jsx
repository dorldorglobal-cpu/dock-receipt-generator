...
function TowingVerifyForm({ verify, orderId, onDone }) {
  ...
  const confirm = async () => {
    setSaving(true);
    try {
      const res = await fetch(`http://localhost:4000/api/orders/${orderId}/confirm-towing-cost`, {
        method: "POST",
...
export default function OrderDetails() {
  ...
  useEffect(() => {
    fetch("http://localhost:4000/api/schedule/vessels")
  ...
  const fetchOrderInvoices = async () => {
    try {
      const res  = await fetch(`http://localhost:4000/api/invoices/by-order/${id}`);
      const data = await res.json();
    ...
    try {
      const [towingRates, oceanRates] = await Promise.all([
        ((needsTowing || needsTowingCost) && pickupCity && pol)
          ? fetch("http://localhost:4000/api/pricing?type=towing").then(r => r.json())
          : Promise.resolve([]),
        ((needsOcean || needsOceanCost) && pol && pod)
          ? fetch("http://localhost:4000/api/pricing?type=ocean").then(r => r.json())
          : Promise.resolve([]),
      ]);
  ...
  const fetchDriveFiles = async () => {
    try {
      const res  = await fetch(`http://localhost:4000/api/orders/${id}/drive-files`);
      const data = await res.json();
  ...
  const fetchVoyages = async () => {
    const res = await fetch(`http://localhost:4000/api/orders/voyages/all`);
    const data = await res.json();
    setVoyages(data);
  };
  ...
  const fetchBills = async (refNumber) => {
    ...
    try {
      const res  = await fetch(`http://localhost:4000/api/expenses?orderRef=${encodeURIComponent(ref)}`);
      const data = await res.json();
  ...
  const saveBill = async () => {
    ...
    try {
      ...
      const url    = editingBill ? `http://localhost:4000/api/expenses/${editingBill._id}` : "http://localhost:4000/api/expenses";
  ...
  const removeBillFile = async (type) => {
    ...
    await fetch(`http://localhost:4000/api/expenses/${editingBill._id}/${type}`, { method: "DELETE" });
    setEditingBill(b => ({ ...b, [`${type}FileName`]: "" }));
  };

  const markBillPaid = async (billId) => {
    const res = await fetch(`http://localhost:4000/api/expenses/${billId}/pay`, {
      method:  "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  const deleteBill = async (billId) => {
    ...
    const res = await fetch(`http://localhost:4000/api/expenses/${billId}`, { method: "DELETE" });
    if (res.ok) fetchBills();
  };
  ...
  const moveToVoyage = async (voyage) => {
    ...
    const res = await fetch(`http://localhost:4000/api/orders/${id}/move-to-voyage`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  const createVoyageAndMove = async () => {
    ...
    const res = await fetch(`http://localhost:4000/api/orders/voyages/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        shippingLine,
  ...
  const clearVoyage = async () => {
    ...
    const res = await fetch(`http://localhost:4000/api/orders/${id}/clear-voyage`, {
      method: "POST",
    });
  ...
  const uploadFile = async (file, label) => {
    ...
    try {
      ...
      const res  = await fetch(`http://localhost:4000/api/orders/${id}/upload-drive`, {
        method: "POST",
        body:   fd,
      });
      const data = await res.json();
  ...
  const deleteFile = async (driveFileId, fileName) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:4000/api/orders/${id}/files/${driveFileId}?name=${encodeURIComponent(fileName)}`,
        { method: "DELETE" }
      );
  ...
  const parseExistingDriveFiles = async () => {
    ...
    const res = await fetch(`http://localhost:4000/api/orders/${id}/parse-drive-files`, {
      method: "POST",
    });
  ...
  const openDrPreview = async (forceRefresh = false) => {
    ...
    setDrLoading(true);
    try {
      const res  = await fetch(`http://localhost:4000/api/orders/${id}/dr-payload`);
      const data = await res.json();
      setDrPayload(data);
  ...
  const sendDrEmail = async () => {
    ...
    try {
      ...
      await fetch(`http://localhost:4000/api/orders/${id}/timeline`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const generateInvoicePdf = async () => {
    ...
    try {
      ...
      if (orderInvoices.length > 0) {
        ...
        const updateRes  = await fetch(`http://localhost:4000/api/invoices/${existingId}`, {
          method:  "PUT",
          headers: { "Content-Type": "application/json" },
          body:    JSON.stringify(payload),
        });
        ...
        const createRes = await fetch("http://localhost:4000/api/invoices", {
          method:  "POST",
          headers: { "Content-Type": "application/json" },
      ...
      // Download the PDF
      const a = document.createElement("a");
      a.href     = `http://localhost:4000/api/invoices/${inv._id}/pdf`;
      ...
      const pdfRes = await fetch(`http://localhost:4000/api/invoices/${inv._id}/pdf`);
  ...
  const sendInvoiceEmail = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:4000/api/invoices/${invoiceSendModal.invoiceId}/send`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
          ...
          {orderInvoices.map(inv => {
            ...
            return (
              <div key={inv._id} style={{
                ...
                <button onClick={() => window.open(`http://localhost:4000/api/invoices/${inv._id}/pdf`, "_blank")}
                ...
                {inv.status !== "paid" && (
                  <button onClick={async () => {
                    await fetch(`http://localhost:4000/api/invoices/${inv._id}/status`, {
            ...
            {driveFiles.map((f) => {
              ...
              const createBillFromDoc = async (docLabel) => {
                try {
                  ...
                  const res = await fetch("http://localhost:4000/api/expenses/parse-dispatch-url", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
              ...
              <tbody>
                ...
                {(() => {
                  ...
                  const lookupOceanPrice = async (categoryOverride) => {
                    ...
                    try {
                      const rates = await fetch(`http://localhost:4000/api/pricing?type=ocean`).then(r => r.json());
              ...
              {bills.map(bill => (
                ...
                <tr>
                  ...
                  <td style={{ whiteSpace:"nowrap" }}>
                    {bill.billFileName
                      ? <a href={`http://localhost:4000/api/expenses/${bill._id}/bill`} target="_blank" rel="noreferrer"
                          title="Bill document" style={{ fontSize:16, textDecoration:"none", marginRight:6 }}>📄</a>
                      : null}
                    {bill.receiptFileName
                      ? <a href={`http://localhost:4000/api/expenses/${bill._id}/receipt`} target="_blank" rel="noreferrer"
                ...
                <div style={{ display:"flex", gap:10, marginTop:10 }}>
                  <button type="button"
                    ...
                    onClick={async () => {
                      setBillParsing(true);
                      try {
                        const r = await fetch("http://localhost:4000/api/vendors/parse-document", {
                          method:  "POST",
                          headers: { "Content-Type": "application/json" },
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
                  <DropZone
                    ...
                    existingUrl={editingBill?.billFileName
                      ? `http://localhost:4000/api/expenses/${editingBill._id}/bill` : null}
                  ...
                  <DropZone
                    ...
                    existingUrl={editingBill?.receiptFileName
                      ? `http://localhost:4000/api/expenses/${editingBill._id}/receipt` : null}
