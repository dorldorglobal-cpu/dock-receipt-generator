...
function TowingVerifyForm({ verify, orderId, onDone }) {
  ...
  const confirm = async () => {
    setSaving(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/orders/${orderId}/confirm-towing-cost`, {
        method: "POST",
...
export default function OrderDetails() {
  ...
  const applyScheduleResult = async (params) => {
    ...
    const res  = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/schedule/lookup?${qs}`);
    const data = await res.json();
    if (data.found) {
      await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/orders/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          vessel:      data.vessel,
  ...
  const fetchOrderInvoices = async () => {
    try {
      const res  = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/invoices/by-order/${id}`);
      const data = await res.json();
  ...
  const fetchOrder = async () => {
    const res = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/orders/${id}`);
    const data = await res.json();
    ...
    try {
      ...
      fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/orders/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const fetchDriveFiles = async () => {
    try {
      const res  = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/orders/${id}/drive-files`);
      const data = await res.json();
  ...
  const fetchVoyages = async () => {
    const res = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/orders/voyages/all`);
    const data = await res.json();
    setVoyages(data);
  };
  ...
  const fetchBills = async (refNumber) => {
    ...
    try {
      const res  = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/expenses?orderRef=${encodeURIComponent(ref)}`);
      const data = await res.json();
  ...
  const saveBill = async () => {
    ...
    try {
      ...
      const url    = editingBill ? `${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/expenses/${editingBill._id}` : "${import.meta.env.VITE_API_URL || "http://localhost:4000"}/api/expenses";
  ...
  const removeBillFile = async (type) => {
    ...
    await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/expenses/${editingBill._id}/${type}`, { method: "DELETE" });
    setEditingBill(b => ({ ...b, [`${type}FileName`]: "" }));
  };

  const markBillPaid = async (billId) => {
    const res = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/expenses/${billId}/pay`, {
      method:  "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  const deleteBill = async (billId) => {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/expenses/${billId}`, { method: "DELETE" });
    if (res.ok) fetchBills();
  };
  ...
  const moveToVoyage = async (voyage) => {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/orders/${id}/move-to-voyage`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  const createVoyageAndMove = async () => {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/orders/voyages/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        shippingLine,
  ...
  const clearVoyage = async () => {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/orders/${id}/clear-voyage`, {
      method: "POST",
    });
  ...
  const uploadFile = async (file, label) => {
    ...
    try {
      ...
      const res  = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/orders/${id}/upload-drive`, {
        method: "POST",
        body:   fd,
      });
      const data = await res.json();
  ...
  const deleteFile = async (driveFileId, fileName) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/orders/${id}/files/${driveFileId}?name=${encodeURIComponent(fileName)}`,
        { method: "DELETE" }
      );
  ...
  const parseExistingDriveFiles = async () => {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/orders/${id}/parse-drive-files`, {
      method: "POST",
    });
  ...
  const openDrPreview = async (forceRefresh = false) => {
    ...
    setDrLoading(true);
    try {
      const res  = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/orders/${id}/dr-payload`);
      const data = await res.json();
      setDrPayload(data);
  ...
  const sendDrEmail = async () => {
    ...
    try {
      ...
      await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/orders/${id}/timeline`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const generateInvoicePdf = async () => {
    ...
    try {
      ...
      if (orderInvoices.length > 0) {
        ...
        const updateRes  = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/invoices/${existingId}`, {
          method:  "PUT",
          headers: { "Content-Type": "application/json" },
          body:    JSON.stringify(payload),
        });
      ...
      // Download the PDF
      const a = document.createElement("a");
      a.href     = `${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/invoices/${inv._id}/pdf`;
      ...
      const pdfRes = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/invoices/${inv._id}/pdf`);
  ...
  const sendInvoiceEmail = async () => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/invoices/${invoiceSendModal.invoiceId}/send`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
          ...
          {orderInvoices.map(inv => {
            ...
            return (
              <div key={inv._id} style={{
                ...
                <button onClick={() => window.open(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/invoices/${inv._id}/pdf`, "_blank")}
                ...
                {inv.status !== "paid" && (
                  <button onClick={async () => {
                    await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/invoices/${inv._id}/status`, {
              ...
              <tbody>
                ...
                {(() => {
                  ...
                  const lookupOceanPrice = async (categoryOverride) => {
                    ...
                    try {
                      const rates = await fetch(`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/pricing?type=ocean`).then(r => r.json());
              ...
              {bills.map(bill => (
                ...
                <tr>
                  ...
                  <td style={{ whiteSpace:"nowrap" }}>
                    {bill.billFileName
                      ? <a href={`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/expenses/${bill._id}/bill`} target="_blank" rel="noreferrer"
                          title="Bill document" style={{ fontSize:16, textDecoration:"none", marginRight:6 }}>📄</a>
                      : null}
                    {bill.receiptFileName
                      ? <a href={`${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/expenses/${bill._id}/receipt`} target="_blank" rel="noreferrer"
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
                  <DropZone
                    ...
                    existingUrl={editingBill?.billFileName
                      ? `${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/expenses/${editingBill._id}/bill` : null}
                  ...
                  <DropZone
                    ...
                    existingUrl={editingBill?.receiptFileName
                      ? `${import.meta.env.VITE_API_URL || "${import.meta.env.VITE_API_URL || "http://localhost:4000"}"}/api/expenses/${editingBill._id}/receipt` : null}
